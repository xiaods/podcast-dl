...
一个命令行工具，自动下载 Podcast 音频并转录为文字（支持中文及 90+ 种语言）。
...
需要 Python 3.10–3.12。推荐使用 [uv](https://github.com/astral-sh/uv) 管理隔离的 Python 环境，体验类似 `npx`。

> **注意**：`faster-whisper` 依赖的 `onnxruntime` 在 macOS x86_64 上目前仅支持 Python ≤ 3.12，安装时需指定 `--python 3.11`。
...
> 首次转录时会自动下载 Whisper 模型文件，缓存在 `~/Library/Caches/podcast-dl/`（macOS）或 `~/.cache/podcast-dl/`（Linux）。
...
| `-l, --language` | 自动检测 | 语言代码，如 `zh`、`en`、`ja` |
